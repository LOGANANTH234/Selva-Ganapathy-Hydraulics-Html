...
function ShareModal({ onClose, machineName }: { onClose: () => void; machineName: string }) {
  const [step, setStep] = useState<"main" | "whatsapp" | "telegram">("main")
      ...
      <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
        <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-sm max-h-[90vh] overflow-y-auto">
          <div className="px-5 py-4 border-b border-gray-200 flex items-center gap-3">
            <button onClick={() => setStep("main")} className="p-1 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5 text-gray-500" /></button>
            ...
            <div>
              <h2 className="text-base font-bold text-gray-900">Share via WhatsApp</h2>
              <p className="text-xs text-gray-500">Attach the PDF manually</p>
            </div>
          </div>
          <div className="p-5 space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Phone number <span className="text-gray-400">(optional — with country code)</span></label>
            ...
            <p className="text-[11px] text-gray-400 italic">Note: WhatsApp doesn't allow direct file sharing from web. Please manually attach the downloaded PDF in the WhatsApp chat.</p>
          </div>
          <div className="px-5 py-4 border-t border-gray-200 flex gap-3">
            <button onClick={() => setStep("main")} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">Back</button>
            <button onClick={handleWhatsAppShare} className="flex-1 px-4 py-2.5 bg-[#25D366] text-white rounded-lg text-sm font-semibold hover:bg-[#20BA5A] flex items-center justify-center gap-2">
        ...
        <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-sm max-h-[90vh] overflow-y-auto">
          <div className="px-5 py-4 border-b border-gray-200 flex items-center gap-3">
            ...
            <div className="w-10 h-10 rounded-full bg-[#0088cc] flex items-center justify-center flex-shrink-0">
              <svg viewBox="0 0 24 24" className="w-6 h-6 fill-white">
                <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.446 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12l-6.869 4.332-2.97-.924c-.644-.203-.658-.644.135-.954l11.593-4.470c.54-.196 1.011.131.832.942z" />
              </svg>
            </div>
            <div>
              <h2 className="text-base font-bold text-gray-900">Share via Telegram</h2>
              <p className="text-xs text-gray-500">Send message and attach PDF</p>
            </div>
          </div>
          <div className="p-5 space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Message</label>
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3}
                className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0088cc] resize-none" />
            </div>
            <p className="text-[11px] text-gray-400 italic">Note: Telegram doesn't allow direct file sharing from web. Please manually attach the downloaded PDF in the Telegram chat.</p>
          </div>
          <div className="px-5 py-4 border-t border-gray-200 flex gap-3">
            <button onClick={() => setStep("main")} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">Back</button>
            <button onClick={handleTelegramShare} className="flex-1 px-4 py-2.5 bg-[#0088cc] text-white rounded-lg text-sm font-semibold hover:bg-[#0077b5] flex items-center justify-center gap-2">
              <svg viewBox="0 0 24 24" className="w-4 h-4 fill-white">
