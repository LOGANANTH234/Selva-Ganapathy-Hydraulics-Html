...
function WhatsAppModal({ onClose, machineName }: { onClose: () => void; machineName: string }) {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-sm max-h-[90vh] overflow-y-auto">
        <div className="px-5 py-4 border-b border-gray-200 flex items-center gap-3">
          ...
          <button onClick={onClose} className="ml-auto p-1 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5 text-gray-500" /></button>
        </div>
        ...
        <div className="px-5 py-4 border-t border-gray-200 flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">Skip</button>
          <button onClick={handleShare} className="flex-1 px-4 py-2.5 bg-[#25D366] text-white rounded-lg text-sm font-semibold hover:bg-[#20BA5A] flex items-center justify-center gap-2">
...
<div class="page">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-3 sm:p-6">
      ...
      {showWhatsApp && <WhatsAppModal onClose={() => setShowWhatsApp(false)} machineName={data.machineName} />}
    </div>
  )
}
